...
class TestFaceRecognition:
    ...
    def test_cpu_fallback_not_triggered_when_gpu_finds_faces(
        self, stub_session: Callable[..., mock.Mock], mocker: MockerFixture
    ) -> None:
        mocker.patch.object(FaceDetector, "load")
        face_detector = FaceDetector("buffalo_s", cache_dir="test_cache")
        ...
        assert faces["gpuFallback"] is False
        ensure_cpu_spy.assert_not_called()
    ...
    def test_cpu_fallback_gpu_zero_cpu_zero_no_warning(
        ...
        session.providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
        face_detector.session = session
        face_detector._cpu_session = stub_session((1, 3, 640, 640), outputs=make_scrfd_heads([]))
    ...
    def test_cpu_fallback_gpu_zero_cpu_finds_faces(
        ...
        session.providers = ["CUDAExecutionProvider", "CPUExecutionProvider"]
        face_detector.session = session
        face_detector._cpu_session = stub_session((1, 3, 640, 640), outputs=make_scrfd_heads([(10, 10, 0.8)]))
    ...
    def test_tiled_detection_runs_one_session_per_tile(
        self, stub_session: Callable[..., mock.Mock], mocker: MockerFixture
    ) -> None:
        mocker.patch.object(FaceDetector, "load")
        face_detector = FaceDetector("buffalo_s", cache_dir="test_cache")
