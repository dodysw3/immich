...
const AdminConfigTilingMinDimWithFacesSchema = z
  .object({
  ...
  .meta({ id: 'AdminConfigTilingMinDimWithFacesDto' });

const AdminConfigTilingTriggersSchema = z
  .object({
    minPass1Faces: z.int().min(1).describe('Minimum pass-1 face count to trigger tiling'),
    minDimWithFaces: AdminConfigTilingMinDimWithFacesSchema,
  })
  .meta({ id: 'AdminConfigTilingTriggersDto' });

const AdminConfigTilingSchema = z
  .object({
    enabled: configBool.describe('Enable tiled face detection for large/group photos'),
    ...
    triggers: AdminConfigTilingTriggersSchema,
  })
  .meta({ id: 'AdminConfigTilingDto' });
  ...
  .object({
    ...
    job: z
      .object({
        ...
        pdfProcessing: AdminConfigJobSettingsSchema,
      ...
      .object({
        ...
        facialRecognition: AdminConfigMachineLearningModelSchema.extend({
          ...
          minFaces: z
            ...
            .meta({ visibility: User }),
          minFaceSize: z.int().min(1).describe('Minimum bounding box pixel dimension for face recognition'),
          tiling: AdminConfigTilingSchema,
